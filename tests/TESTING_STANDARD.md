...
A test is considered done when it defines one clear behavior, sets up its state explicitly, asserts the observable result, cleans up any global or external state it touches, and fails with enough context to identify the broken contract. A test is not done if it depends on collection order, local machine state, real network access, unmanaged `sys.modules` changes, raw `os.environ` mutation, or manual QA for behavior that can reasonably be automated.
