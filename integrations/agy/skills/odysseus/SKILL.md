---
name: odysseus
description: Use when the user asks AGY to read or write Odysseus data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped AGY Agent API. Requires ODYSSEUS_URL and ODYSSEUS_API_TOKEN.
---

# Odysseus AGY Skill

Use this skill when a user asks to interact with Odysseus from AGY (Antigravity).
...
- `ODYSSEUS_API_TOKEN`: Scoped API token created in Odysseus Settings > Integrations > Agents > Connect AGY CLI.

If either value is missing, do not guess credentials. Tell the user to connect AGY CLI in Odysseus Settings > Integrations and expose both values to the terminal session.
...
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py todos list
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py todos add "Follow up"
```
...
```bash
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py emails list 5
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py emails read UID
```

If `/api/codex/capabilities` does not show `email.read: true`, do not inspect email. Ask the user to enable Email read in the AGY Agent settings.
...
```bash
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py GET /api/codex/memory
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
The Cookbook surface lets you reproduce what a human would do in Odysseus → Cookbook: read which serves are running, tail their tmux output to see why they crashed, edit the launch command, relaunch, kill a stuck one.

- `GET /api/codex/cookbook/tasks` — list active serve/download/install tasks. Requires `cookbook:read`.
- `GET /api/codex/cookbook/servers` — list configured servers. Requires `cookbook:read`.
- `GET /api/codex/cookbook/cached?host=<NAME>` — list models already cached. Requires `cookbook:read`.
- `GET /api/codex/cookbook/presets` — list saved serve presets. Requires `cookbook:read`.
- `GET /api/codex/cookbook/output/{session_id}?tail=400` — read last N log lines. Requires `cookbook:read`.
- `POST /api/codex/cookbook/serve` — launch a serve task. Requires `cookbook:launch`.
- `POST /api/codex/cookbook/preset/{name}` — launch saved preset. Requires `cookbook:launch`.
- `POST /api/codex/cookbook/adopt` — register tmux session. Requires `cookbook:launch`.
- `POST /api/codex/cookbook/stop/{session_id}` — kill task tmux session. Requires `cookbook:launch`.

```bash
# Survey what's running
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py cookbook tasks

# Tail failing task
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py cookbook output serve-abc12345 400

# Stop previous attempt
python3 ~/.gemini/skills/odysseus/scripts/odysseus_api.py cookbook stop serve-abc12345
```

## Forbidden Bypass Pattern

If you are about to reach the Odysseus host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass Odysseus Settings and token scopes. Ask the user to enable the relevant AGY Agent tool toggle instead.
