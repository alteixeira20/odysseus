...
from .process_identity import execution_environment, snapshot_process
...
class ProcessService:
    ...
    async def run_command(
        ...
        *,
        ...
        if expected_process_identity:
            approved_snapshot = snapshot_process(
                command,
                root=context.execution_root.path,
                execution_mode=context.execution_mode,
            )
            if approved_snapshot.digest != str(expected_process_identity or ""):
                raise ProcessServiceError(
                    "host process identity changed after approval"
                )
        timeout = min(
        ...
        return {
            ...
            "workspace_boundary": (
                ...
                if context.execution_mode is ExecutionMode.SANDBOXED
                    ...
                    "explicit_process_workspace_write_interim"
                    if context.authority_grant.allows(Capability.PROCESS_WORKSPACE_WRITE)
    ...
    async def run_python(
        ...
        *,
        ...
        if expected_process_identity:
            approved_snapshot = snapshot_process(
                f"{python_executable} -I -c "
                + __import__("shlex").quote(code),
                root=context.execution_root.path,
                execution_mode=context.execution_mode,
            )
            if approved_snapshot.digest != str(expected_process_identity or ""):
                raise ProcessServiceError(
                    "host Python identity changed after approval"
                )
        workspace_revision_before = WORKSPACE_SERVICE.revision(
            context.execution_root.path
        )
