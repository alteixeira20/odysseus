...
ALWAYS_AVAILABLE = frozenset({
    ...
    "update_plan",
    # Structured partial plan updates (single-step add/complete/block/...);
    # same canonical backend as update_plan/todowrite — always reachable
    # for the same reason update_plan is.
    "manage_plan",
})
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "get_workspace": "Return the absolute path of the active workspace folder. File tools are confined to it. Process execution is independently authorized as either a safe workspace shell or a full host shell; selecting a workspace grants neither. Call this first when the user refers to 'the project'/'the code'/'this folder' without giving a path.",
    ...
    "apply_patch": "Apply a staged multi-file transaction to source files ON DISK, with rollback on ordinary commit failure. Individual replacements are atomic; the multi-file collection is not globally crash-atomic. Workspace-confined and returns a diff. Prefer over bash redirects/heredocs/sed.",
    ...
    "manage_plan": "Structured plan management for multi-step work: add/update/complete/block/skip/remove a single step, reorder, or replace the whole plan, without resending the whole checklist. Same underlying plan as `update_plan`/`todowrite`. Use `action=read` to check current steps and their ids before editing one.",
